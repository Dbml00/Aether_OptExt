...
pub fn probe(enable: bool) -> BpfCtx {
    ...
    unsafe {
        ...
        // === 程序1: ringbuf 输出 (11 insns, 用于 exec+fork) ===
        let mut ins1: [u64; 11] = [
            0x00000000000016bf,  // r6=r1
            0x0000000e00000085,  // call 14 (get_pid_tgid)
            0x00000000fffc0a63,  // *(u32*)(r10-4)=r0
            0x0000000000001118,  // lddw r1, ring_fd
            0x0000000000000000,
            0x000000000000a2bf,  // r2=r10
            0xfffffffc00000207,  // r2+=-4
            0x00000004000003b7,  // r3=4
            0x00000000000004b7,  // r4=0
            0x0000008200000085,  // call 130 (ringbuf_output)
            0x0000000000000095,  // exit
        ];
        ins1[3] = 0x18u64 | (1u64 << 8) | (1u64 << 12) | ((rfd as u64 & 0xFFFFFFFF) << 32);

        let lic: [u8; 4] = [71, 80, 76, 0];
        let mut vlog = [0u8; 4096];
        let pa1 = P { pt: 5, ic: 11, ins: &ins1 as *const _ as u64, lic: lic.as_ptr() as u64, ll: 1, ls: 4096, lb: &mut vlog as *mut _ as u64, kv: 0, pad: 0 };
...
pub fn sync_rules(ctx: &BpfCtx, rules: &[config::Rule]) {
    ...
    for r in rules {
        ...
        if set_rule(ctx, &comm, mask) {
            // 调试: 只打印部分关键线程
            if r.prio >= 500 || n < 5 {
                info!("eBPF 规则: comm={:?}(len={}) mask={:#x} cpus={}", 
                    std::str::from_utf8(&comm[..len.min(12)]).unwrap_or("?"), 
                    len, mask, r.cpus);
            }
            n += 1;
        }
    }
    info!("eBPF: 同步 {} 条规则到内核 (map_fd={})", n, ctx.rule_map_fd);
}
